...
export default function OutageDashboard({
  ...
  const processExcelFile = (file: File | ArrayBuffer): boolean => {
    setIsImporting(true);
  ...
  const parseWorkbook = (workbook: XLSX.WorkBook): boolean => {
  ...
  const handleGithubLoad = async (customUrl?: string) => {
    ...
    const tryLocalFallback = async (): Promise<boolean> => {
      ...
      // 2. Try local OUTAGE_SGO.xlsx served statically
      try {
        const resXlsx = await fetch('/OUTAGE_SGO.xlsx');
        if (resXlsx.ok) {
          const buf = await resXlsx.arrayBuffer();
          if (buf && buf.byteLength > 1000) {
            const ok = processExcelFile(buf);
            if (ok) {
              setShowGithubInput(false);
              setGithubUrl('');
              return true;
    ...
    try {
      ...
      const success = processExcelFile(arrayBuffer);
      if (!success) {
        const fallbackOk = await tryLocalFallback();
        if (!fallbackOk) {
          setImportError('Não foi possível identificar registros válidos na planilha.');
          setIsImporting(false);
        }
